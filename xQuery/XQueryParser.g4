...
// simplifications:
//
// 1. The parser itself doesn't really enforce ws:explicit except for some easyt
...
flworExpr: intermediateClause* returnClause? ;
...
orderSpec: value=exprSingle
         ...
         ;
empty:    kw_greatest | kw_least;
kw_greatest:     KW_GREATEST;
kw_least:       KW_LEAST;
order:    kw_ascending | kw_descending;
kw_ascending:   KW_ASCENDING;
kw_descending:  KW_DESCENDING;

returnClause: KW_RETURN exprSingle (COMMA exprSingle)* ;

quantifiedExpr: quantifier quantifiedVar (COMMA quantifiedVar)*
                KW_SATISFIES value=exprSingle ;
quantifier:     kw_some | kw_every;
...
ifExpr: KW_IF LPAREN conditionExpr RPAREN
        KW_THEN thenExpr
        KW_ELSE elseExpr ;
...
comparisonExpr: stringConcatExprLhs ( (valueComp | generalComp | nodeComp) stringConcatExprRhs )? ;

stringConcatExprRhs: rangeExpr (CONCATENATION rangeExpr)* ;

stringConcatExprLhs: rangeExpr (CONCATENATION rangeExpr)* ;

rangeExpr: additiveExprLhs (KW_TO additiveExprRhs)? ;

additiveExprLhs: multiplicativeExpr ( (kwPlus | kwMinus) multiplicativeExpr )* ;

additiveExprRhs: multiplicativeExpr ( (kwPlus | kwMinus) multiplicativeExpr )* ;

kwPlus:       PLUS;

kwMinus:      MINUS;

multiplicativeExpr: unionExpr ( (kwStar | kwDiv | kwIdIv | kwMod) unionExpr )* ;
...
unionExpr: intersectExceptExpr ( (kwUnion | kwVbar) intersectExceptExpr)* ;

kwUnion:       KW_UNION;

kwVbar:        VBAR;

intersectExceptExpr: instanceOfExpr ( (kwIntersect | kwExcept) instanceOfExpr)* ;

kwIntersect:        KW_INTERSECT;
...
unaryExpression: (kwMinus | kwPlus)* valueExpr ;

valueExpr: validateExpr | extensionExpr | simpleMapExpr ;

generalComp: equal | notEqual | langle| langleEqual | rangle | rangleEqual ;
...
valueComp: kwEqual | kwNotEqual | kwLessThan | kwLessThanEqual | kwGreaterThan | kwGreaterThanEqual ;
...
kwLessThanEqual: KW_LE;

kwGreaterThanEqual: KW_GE;

nodeComp: kwIs | langleLangle | rangleRangle ;
...
pathExpr: (kwSlash relativePathExpr?) | (kwDSlash relativePathExpr) | relativePathExpr ;
...
argument: exprSingle | symQuestion ;

symQuestion:    QUESTION;
...
dirElemConstructorOpenClose:  open_langle
                             dirElemContent*
                             close_langle;

open_langle:                LANGLE openName=qName dirAttributeList endOpen=RANGLE;
close_langle:               startClose=LANGLE slashClose=SLASH closeName=qName RANGLE;
